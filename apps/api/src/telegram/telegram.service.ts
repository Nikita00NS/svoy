...
export class TelegramService {
  ...
  private async handleCallback(callbackQuery: any) {
    ...
    if ((data.startsWith('approve_login:') || data.startsWith('deny_login:')) && fromId !== ownerId) {
      await this.telegramApi.answerCallbackQuery(callbackQuery.id, 'Только владелец может подтвердить вход');
      return { ok: true };
    }

    if (data.startsWith('approve_login:')) {
      const token = data.replace('approve_login:', '');
      const approved = await this.authService.approveBotLogin(token);
      await this.telegramApi.answerCallbackQuery(callbackQuery.id, approved ? 'Вход подтвержден' : 'Запрос входа истек');
      await this.telegramApi.sendMessage(chatId, approved ? '✅ Вход в админ-панель подтвержден.' : '⏰ Запрос входа уже недействителен.');
      return { ok: true };
    }

    if (data.startsWith('deny_login:')) {
      const token = data.replace('deny_login:', '');
      const denied = await this.authService.denyBotLogin(token);
      await this.telegramApi.answerCallbackQuery(callbackQuery.id, denied ? 'Вход отклонен' : 'Запрос уже недействителен');
      await this.telegramApi.sendMessage(chatId, denied ? '❌ Вход в админ-панель отклонен.' : '⏰ Запрос входа уже недействителен.');
      return { ok: true };
    }
